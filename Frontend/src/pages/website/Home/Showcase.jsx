          ...
          {newArrivals.map((item) => (
            <SwiperSlide key={item.id}>
              <div className="flex flex-col h-full" data-fly-card>
                <div className="group relative bg-[#d1aaaa8a] dark:bg-white/5 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 rounded-full hover:-translate-y-1">
                  <div className="h-80 flex items-center justify-center overflow-hidden">
                    <img
                      src={item.image}
                      alt={item.name}
                      loading="lazy"
                      className="w-[50%] object-contain transition-transform duration-700 ease-out group-hover:scale-110"
                    />
                  </div>
