...
/**
 * A single Instagram reel card — thumbnail, caption (truncated), publish date,
 * and a "View on Instagram" button. Mirrors VideoCard's structure/animation so
 * the two feeds feel identical; portrait aspect suits reels.
 */
export default function ReelCard({ reel }) {
  const { caption, thumbnail, permalink, publishedAt } = reel;
    ...
    <article className="group relative flex flex-col h-full rounded-3xl overflow-hidden bg-white/80 dark:bg-white/[0.04] border border-white/60 dark:border-white/10 backdrop-blur-xl shadow-[0_10px_40px_-18px_rgba(0,0,0,0.25)] hover:shadow-[0_24px_55px_-18px_rgba(225,48,108,0.4)] hover:-translate-y-1.5 transition-all duration-500">
      <div className="relative aspect-[4/5] overflow-hidden bg-[#fbfefb] dark:bg-white/5">
        {thumbnail && imgOk ? (
          <img
            src={thumbnail}
            alt={caption ? `Reel: ${caption.slice(0, 80)}` : "Instagram reel thumbnail"}
            loading="lazy"
            decoding="async"
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent opacity-70 group-hover:opacity-90 transition-opacity duration-500" />
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <h3 className="font-semibold text-[14px] leading-snug text-gray-900 dark:text-white line-clamp-2 min-h-[2.5rem] group-hover:text-[var(--accent)] transition-colors">
        ...
        <a
          ...
          aria-label={`View reel on Instagram${caption ? `: ${caption.slice(0, 60)}` : ""}`}
